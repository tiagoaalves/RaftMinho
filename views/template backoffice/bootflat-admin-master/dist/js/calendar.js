...
(function ($) {
  ...
  function calendar($el, options) {
    ...
    function draw() {
      ...
      $('.' + (new Date()).toDateCssClass()).addClass('hoje');
  ...
  ;(function (defaults, $, window, document) {
    ...
    shortMonths: ["Jan", "Fev", "Mar", "Abr", "Maio", "Jun", "Jul", "Ago", "Set", "Out", "Nov", "Dec"],
    date: (new Date()),
...
async function getCalendarioAtividades() {
  const atividades = [];
  const resposta = await fetch('/atividade/tudo');
  const { error, response } = await resposta.json();
      
  console.log(JSON.stringify(response))
  for (const atividade of response) {
    ...
    atividades.push(calendarioAtividade);
  }

  atividades.sort(function(a,b) { return (+a.start) - (+b.start); });
  
  return atividades;
}

getCalendarioAtividades().then(data => $('#holder').calendar({ data }));
